...
def cast(sd, precision):
    if precision in ("fp8_e4m3fn", "fp8_e4m3fn_mixed_block35_fp16") and FP8 is None:
        raise SystemExit(
            f"precision {precision!r} requires a PyTorch build providing torch.float8_e4m3fn"
        )
    out = {}
    nvfp4_quantized = 0
    nvfp4_kept_fp16 = 0
